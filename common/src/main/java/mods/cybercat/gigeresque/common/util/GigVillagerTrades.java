...
public record GigVillagerTrades() {
    ...
    public static void addTrades() {
        List<VillagerTrades.ItemListing> gigTrades = new ArrayList<>(
            Arrays.asList(VillagerTrades.TRADES.get(VillagerProfession.CARTOGRAPHER).get(4))
        );
        gigTrades.add(
            new TreasureMapForEmeralds(13, GigTags.GIG_EXPLORER_MAPS, "filled_map.gig_dungeon", MapDecorationTypes.RED_MARKER, 12, 5)
        );
        ...
        List<VillagerTrades.ItemListing> gigTrades2 = new ArrayList<>(
            Arrays.asList(VillagerTrades.TRADES.get(VillagerProfession.CARTOGRAPHER).get(5))
        );
        gigTrades2.add(
            new TreasureMapForEmeralds(13, GigTags.GIG_EXPLORER_MAPS, "filled_map.gig_dungeon", MapDecorationTypes.RED_MARKER, 12, 5)
        );
