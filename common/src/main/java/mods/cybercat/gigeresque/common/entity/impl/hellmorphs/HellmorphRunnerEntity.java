 ...
 */
public class HellmorphRunnerEntity extends AlienEntity implements SmartBrainOwner<HellmorphRunnerEntity> {
    ...
    @Override
    public List<ExtendedSensor<HellmorphRunnerEntity>> getSensors() {
        return ObjectArrayList.of(new NearbyPlayersSensor<>(),
                new NearbyLivingEntitySensor<HellmorphRunnerEntity>().setPredicate(
                        GigEntityUtils::entityTest),
                new NearbyBlocksSensor<HellmorphRunnerEntity>().setRadius(7),
                new NearbyRepellentsSensor<HellmorphRunnerEntity>().setRadius(15).setPredicate(
                        (block, entity) -> block.is(GigTags.ALIEN_REPELLENTS) || block.is(Blocks.LAVA)),
                new NearbyLightsBlocksSensor<HellmorphRunnerEntity>().setRadius(7).setPredicate(
                        (block, entity) -> block.is(GigTags.DESTRUCTIBLE_LIGHT)), new HurtBySensor<>(),
                new UnreachableTargetSensor<>(), new HurtBySensor<>());
    }

    @Override
    public BrainActivityGroup<HellmorphRunnerEntity> getCoreTasks() {
    ...
    @Override
    public BrainActivityGroup<HellmorphRunnerEntity> getFightTasks() {
