...
public class HellbursterEntity extends RunnerbursterEntity implements Growable {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return LivingEntity.createLivingAttributes().add(Attributes.MAX_HEALTH,
                        CommonMod.config.hellbusterConfigs.hellbusterHealth).add(Attributes.ARMOR, 0.0f).add(
                ...
                .add(Attributes.FOLLOW_RANGE, 32.0).add(Attributes.MOVEMENT_SPEED, 0.3300000041723251).add(Attributes.ATTACK_DAMAGE,
                        CommonMod.config.hellbusterConfigs.hellbusterAttackDamage).add(Attributes.ATTACK_KNOCKBACK, 0.3);
    }
    ...
    public LivingEntity growInto() {
        LivingEntity alien;
        if (this.getRandom().nextInt(0, 100) >= 51) alien = GigEntities.BAPHOMORPH.get().create(level());
        else alien = GigEntities.HELLMORPH_RUNNER.get().create(level());
