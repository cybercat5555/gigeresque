...
public record GigMeleeAttackSelector() {
    ...
    public static final AnimationSelector<PopperEntity> POPPER_SELECTOR = popperEntity -> {
        if (popperEntity.getTarget() != null) {
            var vec3d2 = new Vec3(popperEntity.getTarget().getX() - popperEntity.getX(), 0.0,
    ...
    public static final AnimationSelector<SpitterEntity> SPITTER_RANGE_SELECTOR = spitterEntity -> spitterEntity.triggerAnim(Constants.ATTACK_CONTROLLER, Constants.ACID_SPIT);
}
