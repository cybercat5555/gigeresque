...
public class RomAlienEntity extends AlienEntity implements SmartBrainOwner<RomAlienEntity> {
    ...
    public List<ExtendedSensor<RomAlienEntity>> getSensors() {
        return ObjectArrayList.of(
                ...
                // Living Sensor
                new NearbyLivingEntitySensor<RomAlienEntity>().setPredicate((target, self) ->  GigEntityUtils.entityTest(target, self) && !target.getType().is(GigTags.GIG_ALIENS) && !this.isPassedOut()),
                // Block Sensor
                new NearbyBlocksSensor<RomAlienEntity>().setRadius(7),
                // Fire Sensor
                new NearbyRepellentsSensor<RomAlienEntity>().setRadius(15).setPredicate(
                        (block, entity) -> block.is(GigTags.ALIEN_REPELLENTS)),
                // Lights Sensor
                new NearbyLightsBlocksSensor<RomAlienEntity>().setRadius(7).setPredicate(
                        (block, entity) -> block.is(GigTags.DESTRUCTIBLE_LIGHT)),
                // Nest Sensor
                new NearbyNestBlocksSensor<RomAlienEntity>().setRadius(30).setPredicate(
