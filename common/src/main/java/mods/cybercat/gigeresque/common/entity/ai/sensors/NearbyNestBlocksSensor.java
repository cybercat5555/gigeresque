...
public class NearbyNestBlocksSensor<E extends LivingEntity> extends PredicateSensor<BlockState, E> {

    private static final List<MemoryModuleType<?>> MEMORIES = ObjectArrayList.of(GigMemoryTypes.NEARBY_NEST_BLOCKS.get());

    protected SquareRadius radius = new SquareRadius(1, 1);

    public NearbyNestBlocksSensor() {
    ...
    public SensorType<? extends ExtendedSensor<?>> type() {
        return GigSensors.NEARBY_NEST_BLOCKS.get();
    }
     ...
     * @return this
     */
    public NearbyNestBlocksSensor<E> setRadius(double radius) {
     ...
     * @return this
     */
    public NearbyNestBlocksSensor<E> setRadius(double xz, double y) {
    ...
    protected void doTick(ServerLevel level, E entity) {
        ...
        if (blocks.isEmpty())
            BrainUtils.clearMemory(entity, GigMemoryTypes.NEARBY_NEST_BLOCKS.get());
        else
            BrainUtils.setMemory(entity, GigMemoryTypes.NEARBY_NEST_BLOCKS.get(), blocks);
    }
}
