...
public class AlienPanic extends Behavior<PathfinderMob> {
    ...
    private static final Predicate<PathfinderMob> DEFAULT_SHOULD_PANIC_PREDICATE = pathfinderMob -> pathfinderMob.getLastAttacker() != null
    ...
    protected void tick(@NotNull ServerLevel serverLevel, PathfinderMob pathfinderMob, long l) {
        ...
        if (attacker != null) {
            ...
            var runAwayDirection = mobPos.subtract(attackerPos).normalize().scale(30.0); // Scale to desired run
                                                                                         // distance
